...
|<\\/((?:[\\w-]+:)?[a-zA-Z][\\w-]*)\\s*>`, 'gi' );
	...
	private static readonly _regexpDequote: RegExp = /^"|"$|\\(?=")/g;
	private static readonly _regexpCanBacktick: RegExp = /`|\${/;
	...
	private static _Quote( value: string ): string {
		let returnValue: string = JSON.stringify( value );

		if( value.includes( '"' ) ) {
			const content: string = returnValue.replace( jTDAL._regexpDequote, "" );
			if( !value.includes( "'" ) ) {
				returnValue = "'" + content + "'";
